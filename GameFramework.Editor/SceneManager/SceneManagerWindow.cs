...
namespace BayatGames.Utilities.Editor
{
    ...
    public class SceneManagerWindow : EditorWindow
    {
        ...
        protected string[] tabs = new string[] {
            "AllScenes",
            "Scenes in Categories",
            "Settings"
        };
        protected string lastScene;

        [MenuItem("Window/Game Framework/Scene Manager")]
        ...
        protected virtual void ScenesInCategoriesTabGUI()
        {
            var levelScenes = GameFrameworkSettings.GameDatabase.GetCategorizedScenes(3);
            var worldScenes = GameFrameworkSettings.GameDatabase.GetCategorizedScenes(2);

            var categorizedScenes = new List<CategoryAndScene>(levelScenes);
            categorizedScenes.AddRange(worldScenes);

            List<EditorBuildSettingsScene> buildScenes = new List<EditorBuildSettingsScene>(EditorBuildSettings.scenes);
            //guids = categorizedScenes.ToArray();

            if (this.selectedScenes == null || this.selectedScenes.Length != categorizedScenes.Count)
            {
                this.selectedScenes = new bool[categorizedScenes.Count];
            }
            ...
            for (int i = 0; i < categorizedScenes.Count; i++)
            {
                /* Already stored the path */
                string path = categorizedScenes[i].scenePath;
                ...
                GUIStyle style = isOpen ? EditorStyles.boldLabel : EditorStyles.wordWrappedLabel;
                style.richText = true;
                if (isOpen)
                {
                    GUILayout.Label($"{sceneAsset.name} (<color=#005555>{categorizedScenes[i].category}</color>)", style);
                }
                else
                {
                    GUILayout.Label($"{sceneAsset.name} (<color=#005555>{categorizedScenes[i].category}</color>)", style);
                ...
                GUI.backgroundColor = isOpen ? new Color(0, 0.9f, 0.9f) : prevColor;
        ...
        protected virtual void ScenesTabGUI()
        {
            ...
            for (int i = 0; i < guids.Length; i++)
            {
                ...
                if (isOpen)
                {
                    GUILayout.Label(sceneAsset.name, style);
                }
                else
                {
                    GUILayout.Label(sceneAsset.name, style);
        ...
        protected void BulkAndGeneralActions()
        {
            ...
            if (GUILayout.Button(new GUIContent("Settings", "Open the 'Build Settings' Window for managing scenes.")))
            {
        ...
        public virtual void Close(Scene scene)
        {
            EditorSceneManager.SaveCurrentModifiedScenesIfUserWantsTo();
